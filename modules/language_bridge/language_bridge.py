...
__metadata__ = {
    ...
    "events": ["bridge.call", "bridge.health", "bridge.reload_config", "bridge.reset_cb"],
}
    ...
    try:
        ...
        if mode == "docker" and runtimes.get("docker"):
            exec_cmd = [
                ...
                "--memory=256m",
                "--cpus=0.5",
                "python:3.11-slim"  # 这里用公共镜像示意，真实项目请换成内部沙箱镜像
            ] + exec_cmd
            cwd = None  # docker 内部工作目录使用镜像默认
...
class LanguageBridgeModule(BaseModule):
    ...
    def preload(self):
        ...
        if hasattr(self.context, "event_bus") and self.context.event_bus:
            ...
            bus.subscribe("bridge.call", self.handle_event)
            bus.subscribe("bridge.health", self.handle_event)
            bus.subscribe("bridge.reload_config", self.handle_event)
            bus.subscribe("bridge.reset_cb", self.handle_event)
    ...
    def handle_event(self, event, *args, **kwargs):
        try:
            ...
            if name == "bridge.call":
                # data: {"name": "go_example", "input": "hello", "timeout": 5}
                bname = data.get("name")
                text = data.get("input", "")
                to = int(data.get("timeout", 5))

                loop = asyncio.get_event_loop()
                result = loop.run_until_complete(BRIDGE_REG[bname](text)) if bname in BRIDGE_REG else f"错误：未知桥接器 {bname}"
    ...
    def action_bridge_call(self, context=None, params=None, **kwargs):
        ...
        loop = asyncio.get_event_loop()
        return loop.run_until_complete(BRIDGE_REG[name](text))
    ...
    def _register_actions(self):
        ...
        log.info("[bridge] 动作注册完成：bridge.call / bridge.health / bridge.reload_config / bridge.reset_cb")
