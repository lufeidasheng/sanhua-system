...
def _sanhua_gui_bridge_public_callable_names(_obj):
    _names = []
    if _obj is None:
        return _names
...
def _sanhua_gui_install_runtime_aicore_bridge(_runtime, _ac):
    ...
    _public = _sanhua_gui_bridge_public_callable_names(_runtime)
    print(
        f"🧭 GUI runtime bridge probe: "
        f"class={_runtime.__class__.__name__} public_callables={_public}"
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="升级 GUI runtime AICore bridge 到 v2")
    ...
    print("=" * 96)
    print("repair_gui_runtime_aicore_bridge_v2")
    print("=" * 96)
    print(f"root   : {root}")
