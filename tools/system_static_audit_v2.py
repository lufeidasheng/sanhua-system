...
能力：
1. 扫描项目目录下的 modules/、模块/、core/、entry/、config/
2. 发现模块、核心组件、入口文件、manifest.json
3. 建立 Python 静态 import / 调用关系图
4. 判断模块是否接入统一调度、GUI 链路、静态入口链路
5. 校验 manifest 基本合规性
6. 识别 aliases.yaml 中的潜在悬挂引用
7. 输出 Markdown + JSON + Graphviz DOT 三类结果

用法：
    python tools/system_static_audit_v2.py --root . --output-dir ./_audit

说明：
- 这是静态分析工具，动态加载、manifest 驱动装配、dispatcher 运行时注册、monkey patch
  等机制不会被完全还原，因此请把 “static_entry_reachability” 理解为静态入口可达性。
"""
...
# 显式路径优先，不再按类名找到文件后把整个父目录都算成一个组件
...
class ProjectAuditor:
    ...
    def run(self) -> Dict[str, Any]:
        ...
        # 顺序很关键：先读 aliases，再发现模块，再做 alias audit
    ...
    def _load_aliases_raw(self) -> None:
        ...
        try:
            ...
            return
        self.alias_map = data if isinstance(data, dict) else {"_raw": data}
    ...
    def _resolve_manifest_entry(self, module_dir: Path, entry_value: str) -> Tuple[bool, str, Optional[str]]:
        ...
        # 2) mod.py:ClassName 或 pkg.mod:ClassName
        if ":" in text:
        ...
        # 3) 纯 Python import path 或 pkg.mod.ClassName
    ...
    def _audit_aliases_after_modules(self) -> None:
        ...
        for alias, target in pairs:
            ...
            for mod in self.module_records.values():
                ...
                if mod.name.lower() in target_lower or mod.path.lower() in target_lower or target_lower in search_pool:
            ...
            if matched_module:
                continue
            if known_actionish_names and target_text not in known_actionish_names and target_lower not in {x.lower() for x in known_actionish_names}:
                # 不是路径，也没落到已知模块，也不像已知动作/符号名
        ...
        for rel, node in self.file_nodes.items():
            ...
            for call in node.calls:
                if any(k in call.lower() for k in ACTIONISH_KEYWORDS):
                    names.add(call)
            if rel.endswith("actions.yaml") or rel.endswith("aliases.yaml"):
    ...
    def _score_and_finalize_modules(self) -> None:
        for mod in self.module_records.values():
            ...
            # 对 module 降低“无人依赖 = 死代码”的权重，因为三花聚顶存在动态加载/manifest装配
            ...
            blockers.append(
                {
                    "title": "统一调度接入不完整",
                    "severity": "high",
                    "why": "部分模块具备 action / intent / handler 特征，但未检测到明显通过 dispatch_action / call_action / QuantumActionDispatcher 接入。",
            ...
            blockers.append(
                {
                    "title": "GUI 与模块存在直接耦合",
                    "severity": "high",
                    "why": "检测到模块内部直接依赖 GUI 相关实现，破坏“GUI 只负责显示/输入/转发”的边界。",
    ...
    def _judge_dispatch(self, mod: ModuleRecord, hits: List[str]) -> str:
        ...
        actionish = any(any(k in rel.lower() for k in ACTIONISH_KEYWORDS) for rel in mod.files)
        if not actionish:
            for rel in mod.files:
                node = self.file_nodes.get(rel)
                if not node:
        ...
        return "bypassing_or_missing" if actionish else "unknown"

    def _judge_gui(self, mod: ModuleRecord, hits: List[str]) -> str:
        lower_path = mod.path.lower()
        if "entry/gui_entry" in lower_path or lower_path.endswith("gui_main.py") or mod.name.lower().endswith("gui"):
            return "gui_layer"
    ...
    def _append_module_issues(self, mod: ModuleRecord) -> None:
        ...
        if mod.gui_integration == "direct_coupling":
            mod.issues.append("模块内部存在 GUI 直接依赖")
        if mod.kind == "module" and not mod.depended_by:
    ...
    def _discover_entry_candidates(self, module_dir: Path, manifest: ManifestAudit) -> List[str]:
        ...
        if manifest.entry_declared and manifest.entry_exists:
            # 只把能映射为文件的候选加入 entry_candidates
    ...
    def _file_uses_gui(self, node: FileNode) -> bool:
        ...
        return any(k.lower() in joined for k in GUI_KEYWORDS)
    ...
    def _resolve_relative_import(self, current_module_name: str, module: str, level: int) -> str:
        """
        修复点：
        - __init__.py 不能把 __init__ 当普通末尾模块名带入前缀
        - level 表示向上跳的包层数
        """
        if level <= 0:
            return module
    ...
    def _write_markdown(self, payload: Dict[str, Any]) -> None:
        ...
        lines.append("# 三花聚顶系统 · 静态结构与模块关系审计报告")
        lines.append("")
        ...
        for mod in self._sorted_modules():
            ...
            lines.append(f"- alias：`{', '.join(mod.aliases) or '无'}`")
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="三花聚顶系统 · 静态结构与模块关系审计脚本 v2")
