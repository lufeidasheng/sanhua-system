...
class _HealthAdapter:
    """把 HealthMonitorPlus 适配成 aicore_check 期望的 get_health_status()"""
...
class _ControllerCompat:
    """
    兼容层：提供 recent_logs(kind, lines)，用于 aicore_check.py
    优先尝试 /logs 或 /health，不行则返回空列表。
    """
...
class ExtensibleAICore:
    ...
    def _sanitize_llm_output(self, text: str) -> str:
        ...
        清洗模型原始输出：
        - 去掉完整 <think>...</think>
        - 如果只有未闭合 <think> 片段，直接清空
        - 去掉 analysis/final 控制标记
        """
        if text is None:
            return ""
        ...
        # 3) 如果存在 final 标记，优先截取 final 后正文
