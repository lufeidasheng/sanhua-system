...
def build_helper_block() -> str:
    lines = [
        ...
        r'    _memory_block = "\n".join(_lines).strip()',
        "    return (",
        r'        "请把下面这些系统记忆当作高优先级参考事实。\n"',
        r'        "它们用于回答与用户身份、历史对话、刚才说过的话、长期偏好相关的问题。\n"',
        r'        "如果无关就忽略，不要硬编。\n\n"',
        r'        f"{_memory_block}\n\n"',
        r'        f"当前用户问题：\n{_user_text}"',
        "    )",
...
def replace_helper_block(text: str) -> str:
    block = build_helper_block().strip()
...
def patch_text(before: str) -> tuple[str, list[str]]:
    text = replace_helper_block(before)
    text, refs = ensure_install_call(text)
    return text, refs


def main() -> int:
    parser = argparse.ArgumentParser(description="修复 GUI memory pipeline 运行桥接（v3）")
    ...
    print("=" * 96)
    print("repair_gui_memory_pipeline_runtime_v3")
    print("=" * 96)
    print(f"root   : {root}")
