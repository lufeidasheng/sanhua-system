...
REPLACEMENT = '''
    ...
    def _augment_backend_status_with_runtime_truth(
        ...
        backend_status["runtime_truth"] = truth

        for backend_name, entry in list(backend_status.items()):
            if backend_name == "runtime_truth":
                continue
...
def main() -> None:
    ...
    pattern = re.compile(
        r"(?s)    def get_status\(self\) -> Dict\[str, Any\]:\n.*?(?=    def debug_memory_prompt\()"
    )
    match = pattern.search(source)
    if not match:
        raise SystemExit("未匹配到 get_status 区块，补丁终止。")
    ...
    print("✅ backend truth status patch 完成并通过语法检查")
