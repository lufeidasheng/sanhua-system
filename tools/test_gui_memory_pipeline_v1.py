...
def exec_action(dispatcher, name: str, **kwargs):
    if dispatcher is None:
        return {}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="测试 GUI memory pipeline 是否真正注入到 chat/ask")
    ...
    print("=" * 96)
    print("TEST GUI MEMORY PIPELINE V1")
    print("=" * 96)
    print(f"root     : {root}")
    ...
    print("\n[import gui module]")
    mod = load_module_from_path("sanhua_gui_main_for_memory_test_v1", gui_main)
    ...
    def probe_chat(user_input, *args, **kwargs):
        ...
        return {"reply": f"probe-ok: {str(user_input)[:120]}"}
    ...
    hit_identity = ("用户名：" in prompt_1) or ("稳定身份记忆" in prompt_1) or ("鹏" in prompt_1)
    case_results.append(("case_identity", hit_identity, prompt_1[:600], result_1))
    ...
    case_results.append(("case_recent_recall", hit_recent, prompt_2[:1000], None))

    snap = exec_action(dispatcher, "memory.snapshot")
    recent = recent_messages_from_snapshot(snap)
    hit_snapshot = any(marker in x.get("content", "") for x in recent)
    case_results.append(("case_snapshot_writeback", hit_snapshot, recent[-6:], None))
