...
v2.1 修正：
1. 强化排除规则，避免虚拟环境 / 审计输出 / 导出包污染
2. 增加路径级噪音过滤器
3. 排除 _legacy_disabled / fix_backups / rollback_snapshots_runtime
4. 保留 v2 的：
...
class ProjectAuditor:
    ...
    def _should_skip_dirpath(self, path: Path) -> bool:
        rel = "/" + self._relative_or_none(path).strip("/") + "/" if self._relative_or_none(path) else "/"
    ...
    def _build_blockers(self) -> List[Dict[str, Any]]:
        ...
        blockers.sort(key=lambda x: {"critical": 3, "high": 2, "medium": 1}.get(x.get("severity", "medium"), 0), reverse=True)
        return blockers[:3]
    ...
    def _write_markdown(self, payload: Dict[str, Any]) -> None:
        ...
        lines.append("# 三花聚顶系统 · 静态结构与模块关系审计报告 v2.1")
        lines.append("")
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="三花聚顶系统 · 静态结构与模块关系审计脚本 v2.1")
