...
TEST_FILE = Path("tools/test_degraded_negative_memory.py")
...
if "ExtensibleAICore" in globals():
    ...
    if callable(_orig_chat) and not getattr(_orig_chat, "__sanhua_degraded_wrapped__", False):
...
from pathlib import Path as _sanhua_bridge_Path
import json as _sanhua_bridge_json
import re as _sanhua_bridge_re


def _sanhua_bridge_project_root():
    return _sanhua_bridge_Path(__file__).resolve().parents[2]


def _sanhua_bridge_degraded_patterns_path():
    return _sanhua_bridge_project_root() / "data" / "memory" / "degraded_patterns.json"


def _sanhua_bridge_normalize_question(text):
    s = str(text or "").strip().lower()
    if not s:
        return ""
    s = _sanhua_bridge_re.sub(r"\s+", "", s)
    s = _sanhua_bridge_re.sub(r"[\"'`“”‘’\[\]\(\){}<>:：,，。！？!?\-_/\\\\|]+", "", s)
    return s[:500]


def _sanhua_bridge_load_patterns():
    path = _sanhua_bridge_degraded_patterns_path()
    if not path.exists():
        return []

    try:
        payload = _sanhua_bridge_json.loads(path.read_text(encoding="utf-8"))
        patterns = payload.get("patterns", [])
...
def _sanhua_bridge_match_patterns(user_input, top_k=2):
    normalized = _sanhua_bridge_normalize_question(user_input)
    if not normalized:
        return []

    scored = []
    for item in _sanhua_bridge_load_patterns():
        qn = str(item.get("normalized", "")).strip()
        if not qn:
            continue
...
def _sanhua_bridge_inject_degraded_risk_block(self, final_prompt, user_input):
    ...
    matches = _sanhua_bridge_match_patterns(user_input)
    if not matches:
        return prompt
...
if "PromptMemoryBridge" in globals():
    ...
    if not hasattr(_SANHUA_BRIDGE_CLS, "_inject_degraded_risk_block"):
        setattr(_SANHUA_BRIDGE_CLS, "_inject_degraded_risk_block", _sanhua_bridge_inject_degraded_risk_block)

    _orig_build_prompt = getattr(_SANHUA_BRIDGE_CLS, "build_prompt", None)
    if callable(_orig_build_prompt) and not getattr(_orig_build_prompt, "__sanhua_degraded_wrapped__", False):
        ...
        _wrapped_build_prompt.__sanhua_degraded_wrapped__ = True
    ...
    if callable(_orig_build_prompt_payload) and not getattr(_orig_build_prompt_payload, "__sanhua_degraded_wrapped__", False):
        def _wrapped_build_prompt_payload(self, *args, **kwargs):
            ...
            if isinstance(payload, dict):
                ...
                user_input = payload.get("user_input", kwargs.get("user_input", ""))
                if isinstance(final_prompt, str):
                    payload = dict(payload)
                    payload["final_prompt"] = self._inject_degraded_risk_block(final_prompt, user_input)
            return payload

        _wrapped_build_prompt_payload.__sanhua_degraded_wrapped__ = True
...
def main() -> None:
    ...
    # 手工写两次 degraded 负样本，避免依赖真实模型输出
    aicore._record_degraded_pattern(query, "test degraded #1")
    aicore._record_degraded_pattern(query, "test degraded #2")

    payload = aicore.build_memory_payload(
        user_input=query,
        session_context={"source": "test_degraded_negative_memory"},
    )
...
def main() -> None:
    ...
    bak_bridge = replace_or_append_block(
        BRIDGE_FILE,
        "# === SANHUA_DEGRADED_RISK_BRIDGE_PATCH_V1_BEGIN ===",
        "# === SANHUA_DEGRADED_RISK_BRIDGE_PATCH_V1_END ===",
        BRIDGE_PATCH,
    ...
    print("✅ degraded negative memory patch 完成并通过语法检查")
    print(f"backup_aicore : {bak_aicore}")
    print(f"backup_bridge : {bak_bridge}")
    print(f"backup_test   : {bak_test}")
    print("下一步运行：")
    print("python3 tools/test_degraded_negative_memory.py")
