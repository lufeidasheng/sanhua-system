...
TARGET_FILE = "core/core2_0/sanhuatongyu/decision_arbiter.py"

TARGET_CODE = r'''from __future__ import annotations
...
def main() -> None:
    parser = argparse.ArgumentParser(description="patch decision arbiter preview safe")
    ...
    print("=" * 72)
    print("decision_arbiter preview-safe 正式补丁开始")
    print("=" * 72)

    backup_file(target, backup_root, root)
    target.write_text(textwrap.dedent(TARGET_CODE).lstrip(), encoding="utf-8")

    print(f"[PATCHED] {target}")
    print("=" * 72)
    print("decision_arbiter preview-safe 正式补丁完成")
    print("=" * 72)
    print("修复点：")
    print(" - code_inserter.preview_* 视为只读预演动作")
    print(" - allow_file_write=False 时仍允许 preview 动作通过")
    print(" - 非 preview 的 code_inserter.* 仍走写入限制")
    print(" - 保留 shell / 网络 / 高危关键词 / 白名单前缀策略")
    print("=" * 72)
    print("下一步建议：")
    print(f'  python3 "{root / "tools" / "test_code_inserter_official.py"}"')
    print("=" * 72)
