...
def _sanhua_gui_mem_collect_context(_aicore, _user_text, _limit=5):
    ...
    if isinstance(_recall, dict):
        for _item in (_recall.get("results") or []):
            _payload["matches"].append(_sanhua_gui_mem_normalize_match(_item))
...
def make_backup(root: Path, target: Path) -> Path:
    ...
    if target.resolve() == backup_path.resolve():
        raise RuntimeError(f"backup_path 与 target 相同，拒绝覆盖: {backup_path}")
...
def inject_helper_block(text: str) -> str:
...
def inject_install_calls(text: str) -> tuple[str, list[str]]:
    ...
    while i < len(lines):
        ...
        if matched_lhs:
            ...
            if not already:
                out.append(
                    f"{matched_indent}_sanhua_gui_install_memory_pipeline({matched_lhs})  {CALL_MARKER}"
                )
...
def patch_text(original: str) -> tuple[str, list[str]]:
    text = inject_helper_block(original)
    text, refs = inject_install_calls(text)
    return text, refs


def main() -> int:
    parser = argparse.ArgumentParser(description="给 GUI 注入 memory pipeline（chat/ask 包装）")
    ...
    print("=" * 96)
    print("patch_gui_memory_pipeline_v1")
    print("=" * 96)
    print(f"root   : {root}")
    ...
    if after == before:
        print("[SKIP] 无需修改，可能已注入")
        return 0

    print(f"[INFO] patched_refs: {refs if refs else '[]'}")
    if not refs:
        print("[WARN] 未发现明确的 AICore 实例赋值点；helper block 已注入，但安装调用可能未落位")

    diff = unified_diff(before, after, target)
    print("[DIFF PREVIEW]")
    print(diff[:12000] if diff else "(none)")
